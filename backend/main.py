...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # all origins are allowed
...
def extract_project_files_from_zip(zip_content: bytes) -> Dict[str, str]:
    ...
    zip_buffer.seek(0) # buffer for zip
...
@app.post("/single-review-stream") # post single review stream 
...
async def project_review_pdf(
    ...
    try:
        ...
        elif action == "DOCUMENTATION": # documentation
            name = "README FILE"
